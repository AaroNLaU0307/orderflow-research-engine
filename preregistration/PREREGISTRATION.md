...
**Status:** APPROVED 2026-07-02, following one required revision to section 6.5
...
ratified Appendix A resolutions, most substantively the section 6.5 rewrite)
**Date:** 2026-07-02
...
- **Replication (pre-registered, conditional):** ETHUSDT USD-margined perpetual
  ...
  BTC (section 6 gates). ETH is never used for discovery.

**Sample period:** 2022-07-01 to 2026-06-30 (48 months), per the original
brief and confirmed feasible in Phase 0 (section 6 of `DATA_AUDIT.md`; ~54 GB
...
| `bookDepth` | daily zips | Descriptive context only. **Not** usable for any confirmatory signal (see section 3, H4/H5). | `timestamp (string, not epoch), percentage, depth, notional`. Archive coverage confirmed to start ~2023-01-01, i.e. **not** available for the first ~6 months of the study period; any descriptive bookDepth exhibit in the final report is captioned "2023-01 onward." |
...
non-stationary: high-activity months (2022 H2, early/late 2024) ran 2-3x the
...
ETHUSDT was sized (section 6 of `DATA_AUDIT.md`) but not schema/header-probed in
...
one signed-return distribution per signal (section 5), so the tested cell count stays
at **4 signals x 5 horizons = 20** (BTC IS), not 8x5.
...
- **(a) Informational null:** the signal fails the BH-FDR gate (section 6.3) -
...
- **(b) Economic null:** the signal clears BH-FDR (real, non-zero information
  ...
  FDR-significant, >=30m horizon reaches 1.5x round-trip cost) or gate 4 (sign
  flips in >=2 of 3 IS segments at the promoted horizon h*, section 6.5). This is a
...
- **(d) Promoted:** clears all four gates in section 6.5 -> proceeds to the Phase 4
  confirmatory backtest, where OOS and ETH are the actual out-of-family tests
  of whether the IS finding replicates (section 7).
...
- **Bars:** 5-minute, UTC epoch-aligned (`:00, :05, :10, ...`). Persisted keyed
  ...
  (section 7): a bar is never used by any detector until it is fully closed.
...
- **Bucket delta** = `buy_vol - sell_vol`, where buy = aggressor buy =
  ...
  `is_buyer_maker == true`.
- **Persisted parquet schema** (per brief section 2.4, restated): one row per
...
- **Warm-up - corrected from the original brief (Phase-1 adjustment, pre-PnL):**
  ...
  **8640-bar (30-day)** standard deviation of the rolling 24-bar delta sum.
  ...
  partially-populated, non-stationary-length sigma window, which is both a
...
- **Segment-boundary purging:** admission is **per-event**, not
  ...
  **all 5** horizon cells for that segment (not selectively kept for shorter
  ...
  signal x segment cell, so cross-horizon comparisons within a cell are never
...
look-ahead-prevention truncation-invariance requirement (brief section 0.3): the
event is only known as of bar t's close, and the earliest tradeable price is
bar t+1's open.

### 6.2 Per-cell estimators

For each of the **4 signals x 5 horizons = 20 BTC in-sample cells**:
...
- **Day-cluster bootstrap p-value** (the concrete implementation of the
  ...
  the event level.
- **Spearman IC** between event magnitude (section 2, per-signal magnitude measure)
...
**The FDR family is exactly the 20 BTC in-sample cells** (4 signals x 5
...
evaluated at the signal's promoted horizon **h\*** - h\* is defined in section 6.5
...
1. **>= 300** deduplicated IS events (else outcome (c), section 2, underpowered).
2. BH-FDR significant (q < 0.10, within the 20-cell family) at **>= 2**
   horizons, **at least one of which is >= 30 minutes** (i.e., h in {6, 12, 48}).
...
for: gate 4's year-consistency check (section 6.4), the Phase 4 confirmatory
backtest's exit horizon, the OOS confirmation, and the ETH replication (section 7).
...
statement in section 2 and does **not** proceed to Phase 4.

### 6.6 Cost model

- **Taker fee:** 5.0 bp per side (Binance USD-M VIP0, no BNB discount -
  deliberately conservative, not necessarily any specific account's actual
  fee tier).
...
- **Entry:** next bar open after the event bar closes (same reference as the
  event study, section 6.1).
- **Exit:** the fixed **promoted horizon h\*** (section 6.5) selected during IS
...
- **Net PnL** = gross signed return - taker fees (both sides) - slippage (both
  sides) - funding accrued over the holding period, per section 6.6.
- **Walk-forward structure:** purge = h\*'s bar length;
  ...
  block length = 5 days, 10,000 reps, per brief section 4.4).
- **DSR / PSR:** computed with N = 140 (section 6.7).
- **ETH replication:** identical frozen pipeline and identical h\* (not
  re-optimized for ETH), Delta = 1 USDT bucket grid, promoted signals only.
  Reported as an independent confirmation-or-failure with its
  own day-cluster bootstrap CI - outside the FDR family (section 6.3), a single
...
One-factor-at-a-time grid, BTC only, over the 4 testable signals x 5 horizons
(20 cells per config):

| Factor | Alternate values |
|---|---|
| Bucket size Delta | 10 USDT, 50 USDT (baseline: 25 USDT) |
| Bar length | 3 minutes, 15 minutes (baseline: 5 minutes) |

4 alternate configs x 20 cells = **80 cells**, reported purely as descriptive
...
are counted in the DSR trial declaration (section 6.7) precisely because they were
...
The warm-up correction in section 5 (2016 -> 8640 bars) is **not** a
...
1. **Segment-boundary purging granularity (section 5). RATIFIED - per-event
   ...
   a multi-year sample.
2. **"Best FDR-significant horizon" (section 6.5 gate 3, section 7 backtest horizon).
   ...
   exists to eliminate. **Current rule (section 6.5):** gate 3 is restructured to
...
3. **H6 magnitude ("volume percentile x |extreme-bucket delta|"). RATIFIED -
...
4. **H2's "median bucket volume of the trailing 96 bars" (section 2, H2). RATIFIED -
...
could have changed a promotion outcome; its *current* form (section 6.5) removes
...
Every operational value in brief sections 3-4 is transcribed above with no
unstated changes, **except**:
- the warm-up correction (section 5 of this document, brief said 2016 bars / 1 week;
  corrected to 8640 bars / 30 days);
- the sensitivity-grid Delta values (section 8 of this document locks Delta=10/50 USDT
  exactly, a minor rounding of the brief's "x0.5/x2" formula against the
...
- gate 3 / promoted-horizon selection (section 6.5 of this document): the brief's
